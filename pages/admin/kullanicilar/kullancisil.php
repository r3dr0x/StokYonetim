...
<style>
    ...
    .user-table th, .user-table td {
        border: 1px solid black; /* Siyah çizgiler */
        padding: 8px;
        text-align: left;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function deleteUser(userId) {
    ...
    xhr.open("POST", "../../../includes/api/admin/admin_user_del.php", true);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          if (response.success) {
            // Kullanıcıyı başarıyla sildikten sonra SweetAlert bildirimi göster
            Swal.fire({
              icon: "success",
              title: "Başarılı!",
    ...
    const data = "email=" + encodeURIComponent(userId);
    xhr.send(data);
  }
});
